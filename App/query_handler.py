...
class QueryHandler:
    """Handles querying the vector store and generating answers using an LLM."""
    
    def __init__(self, vector_store: VectorStore):
        self.vector_store = vector_store

    def query_and_generate_answer(self, query: str) -> str:
        """Query the vector store, retrieve relevant documents, and generate an answer using the LLM."""
        logging.debug(f"Received query: {query}")
        
        # Query the vector store for relevant documents
        retrieved_texts = self.vector_store.query(query)
...
if __name__ == "__main__":
    ...
    vector_store = VectorStore(store_path='./vector_store/index.faiss', embedding_model_name=EMBEDDING_MODEL_NAME)
    ...
    answer = query_handler.query_and_generate_answer(query)
    print("Answer:", answer)

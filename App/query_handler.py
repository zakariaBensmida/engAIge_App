import os
from transformers import pipeline
from typing import List
from App.vector_store import VectorStore  # Adjust the import path as necessary
from dotenv import load_dotenv

# Load environment variables from .env file
load_dotenv()

class QueryHandler:
    def __init__(self, vector_store: VectorStore):
        """
        Initializes the QueryHandler with the given vector store and a language model pipeline.
        Retrieves the LLM model name from the environment variables.
        """
        # Retrieve the LLM model name from the environment variables, defaulting to 'gpt2' if not found
        llm_model_name = os.getenv("LLM_MODEL_NAME", "gpt2")
        
        # Initialize Hugging Face text generation pipeline
        self.llm = pipeline("text-generation", model=llm_model_name)
        
        # Store the vector store for later querying
        self.vector_store = vector_store

    def get_relevant_texts(self, query: str, top_k: int = 5) -> List[str]:
        """
        Retrieves the top-k relevant texts from the vector store based on the input query.
        
        Args:
            query (str): The search query.
            top_k (int): Number of top similar documents to retrieve.
            
        Returns:
            List[str]: A list of relevant text documents.
        """
        relevant_texts = self.vector_store.query(query, k=top_k)
        return relevant_texts

    def get_answer(self, query: str) -> str:
        """
        Generates an answer to the query by retrieving relevant texts and using an LLM to generate a response.
        
        Args:
            query (str): The user's question or query.
            
        Returns:
            str: The generated answer.
        """
        # Retrieve relevant texts from the vector store
        relevant_texts = self.get_relevant_texts(query)
        
        # Combine the relevant texts into a context
        context = "\n".join(relevant_texts)
        
        # Construct the prompt by providing the context and the question
        prompt = f"Context:\n{context}\n\nQuestion: {query}\nAnswer:"
        
        # Generate the answer using the LLM
        response = self.llm(prompt, max_length=150, num_return_sequences=1, temperature=0.7)
        
        # Return the generated answer
        return response[0]['generated_text'].strip()

# Example usage for debugging
if __name__ == "__main__":
    # Initialize the vector store (replace with your actual parameters)
    vector_store = VectorStore(
        store_path='./vector_store/index.faiss',
        embedding_model_name=os.getenv("EMBEDDING_MODEL_NAME")
    )

    # Initialize the QueryHandler
    query_handler = QueryHandler(vector_store=vector_store)

    # Example query
    query = "What is health insurance coverage?"
    
    # Generate the answer to the query
    answer = query_handler.get_answer(query)
    
    # Print the generated answer
    print("Answer:", answer)


